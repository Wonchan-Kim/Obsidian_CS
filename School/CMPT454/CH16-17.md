...
|     |     | Ra  |
| Ra  |     |     |
so first, T2 gains S lock on A, read A. W1(A): T1 goes to the wait queue. 
W2A gets the X lock write.
T2 finishes and releases all locks. 
T1 wakes up gets the X lock on A, write A. 
R3(a) lock on a, write queue. 
R1(a) locks on a, read a
T1 finishes and release all locks
T3 wakes up and gets s lock on A and finishes. 

Deadlock with strict 2pl
![[Pasted image 20251124174003.png]]
...
W1 gets x-lock on A, 
W2 gets X lock on B,
on Rb, T1 goes to the wait queue on B
on Ra, T2 goes to wait queue on A. 
neither of T1 and T2 will release locks before end due to strict 2PL so they wait forever -> deadlock

2PL also doesn't guarantee the deadlock-freedom

Lock manager implementation

Q1. what are we locking?
Q2. how to lock something?
Lock table: a hash table of lock entries.
	Lock entry: OID(key) Mode | List(Xacts holding lock) | List of waiting queue

![[Pasted image 20251124174813.png]]
